...
const BooksPage = () => {
  ...
  const handlerSaveBook = async (newBook: any) => {
    if (updateBook) {
  ...
  return (
    <div className="container">
      ...
      {!showForm && 
          <button className="add" onClick={() => setShowForm(true)}>Add new Book</button>}
      {showForm && <BookForm onSubmit={handlerSaveBook} updateBook={updateBook} onCancel={handlerCancelUpdate} book={updateBook} />}
      <div className="scroll">
        <ListBooks books={books} onDelete={handlerDeleteBook} onUpdate={handlerUpdateBook} />
      </div>
    </div>
  )
}
